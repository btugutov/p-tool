...
var UserAgentApplication = /** @class */ (function () {
    ...
    UserAgentApplication.prototype.loginRedirectHelper = function (account, request, scopes) {
        ...
        this.authorityInstance.resolveEndpointsAsync().then(function () {
            ...
            var serverAuthenticationRequest = new ServerRequestParameters(_this.authorityInstance, _this.clientId, scopes, ResponseTypes.id_token, _this.getRedirectUri(), request.state);
            ...
            _this.redirectErrorHandler(ClientAuthError.createEndpointResolutionError(err.toString), buildResponseStateOnly(reqState));
        });
    };
    /**
    ...
    UserAgentApplication.prototype.acquireTokenSilent = function (request) {
        ...
        return new Promise(function (resolve, reject) {
            ...
            var serverAuthenticationRequest = new ServerRequestParameters(AuthorityFactory.CreateInstance(request.authority, _this.config.auth.validateAuthority), _this.clientId, request.scopes, responseType, _this.getRedirectUri(), request.state);
            // populate QueryParameters (sid/login_hint/domain_hint) and any other extraQueryParameters set by the developer
    ...
    UserAgentApplication.prototype.getCachedToken = function (serverAuthenticationRequest, account) {
        ...
        if (accessTokenCacheItem != null) {
            ...
            if (expired && (expired > Utils.now() + offset)) {
                var idToken = new IdToken(accessTokenCacheItem.value.idToken);
                if (!account) {
    ...
    UserAgentApplication.prototype.renewToken = function (scopes, resolve, reject, account, serverAuthenticationRequest) {
        ...
        var urlNavigate = Utils.urlRemoveQueryStringParameter(serverAuthenticationRequest.createNavigateUrl(scopes), Constants.prompt) + Constants.prompt_none;
        window.renewStates.push(serverAuthenticationRequest.state);
        window.requestType = Constants.renewToken;
    ...
    UserAgentApplication.prototype.saveAccessToken = function (response, authority, parameters, clientInfo) {
        ...
        if (parameters.hasOwnProperty("scope")) {
            ...
            var expiresIn = Utils.expiresIn(parameters[Constants.expiresIn]).toString();
            var accessTokenKey = new AccessTokenKey(authority, this.clientId, scope, clientObj.uid, clientObj.utid);
            var accessTokenValue = new AccessTokenValue(parameters[Constants.accessToken], response.idToken.rawIdToken, expiresIn, clientInfo);
        ...
        // if the response does not contain "scope" - scope is usually client_id and the token will be id_token
            ...
            // Generate and cache accessTokenKey and accessTokenValue
            var accessTokenKey = new AccessTokenKey(authority, this.clientId, scope, clientObj.uid, clientObj.utid);
            var accessTokenValue = new AccessTokenValue(parameters[Constants.idToken], parameters[Constants.idToken], response.idToken.expiration, clientInfo);
            this.cacheStorage.setItem(JSON.stringify(accessTokenKey), JSON.stringify(accessTokenValue));
